...
class TestAIGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ai_generator = AIGenerator(
            api_key="test_key", model="gpt-5", base_url="http://localhost:4141/v1"
        )
    ...
    @patch("ai_generator.OpenAI")
    def test_generate_response_single_round_backward_compatibility(
        ...
        decision_response = MockOpenAIResponse(
    ...
    def test_generate_response_sequential_two_rounds(self, mock_openai_class):
        ...
        first_response = MockOpenAIResponse(
            content=None, tool_calls=[first_tool_call], finish_reason="tool_calls"
        )
        ...
        decision_continue_response = MockOpenAIResponse(
            content=None, tool_calls=[second_tool_call], finish_reason="tool_calls"
        )
        ...
        final_decision_response = MockOpenAIResponse(
            content="Based on both tool results, here's the complete answer"
        )
        ...
        # Verify it completed successfully
        self.assertEqual(
            result, "Based on both tool results, here's the complete answer"
        )
    ...
    def test_convert_tools_to_openai_format(self):
        ...
        openai_tools = self.ai_generator._convert_tools_to_openai_format(
            anthropic_tools
        )
    ...
    def test_tool_execution_error_handling(self, mock_openai_class):
        ...
        decision_response = MockOpenAIResponse(
            content="Answer based on available information"
        )
