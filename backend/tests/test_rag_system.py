...
class TestRAGSystem(unittest.TestCase):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_successful_response(self, mock_ai_generator_class, mock_vector_store_class):
        """Test successful query processing"""
        # Mock AI generator
        mock_ai_gen = Mock()
        mock_ai_gen.generate_response.return_value = "This is a test response about Python."
        ...
        self.assertEqual(call_args.kwargs['tools'], [{"name": "search_course_content", "description": "Search content"}])
        self.assertEqual(call_args.kwargs['tool_manager'], mock_tool_manager)

    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_with_session_management(self, mock_ai_generator_class, mock_vector_store_class):
        ...
        response, sources = rag_sys.query("Follow up question", session_id="test_session")
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_ai_generator_failure(self, mock_ai_generator_class, mock_vector_store_class):
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_tool_execution_failure(self, mock_ai_generator_class, mock_vector_store_class):
        """Test query when tool execution fails within AI generator"""
        # Mock AI generator that simulates the eval() bug
        mock_ai_gen = Mock()
        mock_ai_gen.generate_response.side_effect = SyntaxError("invalid syntax (eval of JSON)")
        mock_ai_generator_class.return_value = mock_ai_gen
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_source_tracking_and_reset(self, mock_ai_generator_class, mock_vector_store_class):
